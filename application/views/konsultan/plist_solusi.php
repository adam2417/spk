...
<form action="" method="post" name="formtambah" id="formtambah" style="margin-top: 10px; <?php if($konfirmasi!=''){ echo 'visibility: visible; height:160px;'; }else{ echo 'visibility: hidden; height:0;'; } ?>">
	<table>
		...
		<tr>
			<td width="40">ID</td>
			<td><input type="text" name="id_solusi" readonly="readonly" value="<?php echo $new_ID; ?>" style="color: #796D5E !important" /></td>
		</tr>
		<tr>
			<td>Desc</td>
			<td><textarea name="desc_solusi" rows="5" cols="40"></textarea></td>
		</tr>
		<tr>
			<td></td>
...
<table style="border-collapse: collapse;" border="1" cellpadding="5">
	...
	{listdata}
	<tr>
		<td><input type="radio" name="plist_id" value="{id_solusi}#{ket}"></td>
		<td>{id_solusi}</td>		
		<td>{ket}</td>
	</tr>
	{/listdata}
...
function post_value(){
	...
	//var pLabel = opener.document.getElementById("lblsolusi");
	opener.document.frmParent.txtsolusi.value = strData[0];
	opener.document.frmParent.txtsolusi_desc.value = strData[1];
